...
public interface IProtected<T> : IProtected where T : class
{
    ...
    /// <param name="requirement">The required data rights for access.</param>
    /// <returns>A queryable collection of the protected entity.</returns>
    IQueryable<T> Secured(int identityId); // for Read
    ...
    Task<bool> HasAccess(T obj, int identityId, RepositoryOperationEnum operation, CancellationToken cancellationToken); // for Write
}
