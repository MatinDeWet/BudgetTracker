        ...
        switch (result.Status)
        {
            ...
            case ResultStatus.NotFound:
                ...
                List<ValidationFailure> notFoundErrors = includeErrorDetails && result.Errors.Any()
                    ? result.Errors.Select(e => new ValidationFailure("Resource", e)).ToList()
                    : new List<ValidationFailure> { new("Resource", "The requested resource was not found") };
            ...
            case ResultStatus.Unauthorized:
                ...
                List<ValidationFailure> unauthorizedErrors = includeErrorDetails && result.Errors.Any()
                    ? result.Errors.Select(e => new ValidationFailure("Authorization", e)).ToList()
                    : new List<ValidationFailure> { new("Authorization", "You are not authorized to access this resource") };
            ...
            case ResultStatus.Forbidden:
                ...
                List<ValidationFailure> forbiddenErrors = includeErrorDetails && result.Errors.Any()
                    ? result.Errors.Select(e => new ValidationFailure("Permission", e)).ToList()
                    : new List<ValidationFailure> { new("Permission", "You do not have permission to access this resource") };
            ...
            case ResultStatus.Conflict:
                ...
                List<ValidationFailure> conflictErrors = includeErrorDetails && result.Errors.Any()
                    ? result.Errors.Select(e => new ValidationFailure("Conflict", e)).ToList()
                    : new List<ValidationFailure> { new("Conflict", "The request conflicts with the current state of the resource") };
            ...
            case ResultStatus.Error:
                ...
                List<ValidationFailure> errorErrors = includeErrorDetails && result.Errors.Any()
                    ? result.Errors.Select(e => new ValidationFailure("Error", e)).ToList()
                    : new List<ValidationFailure> { new("Error", "An error occurred processing your request") };
            ...
            case ResultStatus.CriticalError:
                ...
                List<ValidationFailure> criticalErrors = includeErrorDetails && result.Errors.Any()
                    ? result.Errors.Select(e => new ValidationFailure("CriticalError", e)).ToList()
                    : new List<ValidationFailure> { new("CriticalError", "A critical error occurred processing your request") };
            ...
            case ResultStatus.Unavailable:
                ...
                List<ValidationFailure> unavailableErrors = includeErrorDetails && result.Errors.Any()
                    ? result.Errors.Select(e => new ValidationFailure("Unavailable", e)).ToList()
                    : new List<ValidationFailure> { new("Unavailable", "The service is currently unavailable") };
                await ep.HttpContext.Response.SendErrorsAsync(unavailableErrors);
                break;

            default:
                ep.HttpContext.Response.StatusCode = (int)HttpStatusCode.InternalServerError;
                await ep.HttpContext.Response.SendErrorsAsync(new List<ValidationFailure> { new("Unknown", "An unknown error occurred") });
                break;
        }
    }
}
